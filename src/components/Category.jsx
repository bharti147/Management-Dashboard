...
function Category({ title, widgets }) {
  ...
  for (let i = 0; i < totalSlots; i++) {
    if (i < widgets.length) {
      ...
      slots.push(
        <div key={i} className="w-full sm:w-1/2 lg:w-1/3 p-2">
          <div className="border border-slate-300 shadow-md h-52 bg-white flex flex-col items-center justify-center p-4 rounded-xl">
            <h1 className='bg-inherit text-xl flex items-center text-gray-500 font-bold'>
              Add Widget <FaPlus className="text-sm text-gray-500 bg-inherit mx-2 font-bold" />
            </h1>
          </div>
        </div>
      );
  ...
  return (
    <div className="w-full flex flex-col my-4 h-auto">
      <h2 className="text-xl font-bold mb-4">{title}</h2>
      <div className="flex flex-wrap">
        {slots}
      </div>
